...
The ``truth'' is represented by a model that is fully mechanistic for at least one important process, but we assume that recovering 
...
Criticism often focuses on the assumption that current spatial patterns are in equilibrium with climate (refs?). A less well-recognized issue is that these models are not dynamic; 
