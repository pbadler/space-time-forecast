...
Another approach to addressing the joint importance of spatial and time-series data for making ecological forecasts is joint spatiotemporal modeling...\comment{Ethan, what do you have in mind here? Probably something more than just working with spatiotemporal datasets and accounting for autocorrelation. Mevin has lots of papers with some sort of mechanistic modeling of spatiotemoral processes like dispersal and colonization. Should we say that the datasets needed to train this kind of model are rare, but that might be changing with more and more remote sensing data that is extensive in space and time? }
...
\begin{equation}
	\ln(\frac{N(t+1)}{N(t)}) = T(N(t),K(t)) = \beta^T_0 +  \beta^T_1 ln(N(t)) +\beta^T_2 K(t)  +\beta^T_3 K(t)^2 +  \varepsilon
...
\end{equation}
\comment{They have a delta t term right before the second P', but since this is a discrete time model and our time step always = 1, it seems confusing to add that...}
In the absence of competition, the growth rate ($g$) is determined by the difference between the temperature ($K$) at site $x$ and the focal species' minimum temperature tolerance, $Kmin$, the lowest temperature at which a species can maintain a positive growth rate. Growth is further reduced by intraspecific ($c$) and interspecific ($l$) competition. All species are assigned the same value of $c$, which represents the additional limitation each species experiences from intraspecific competition. This offset also stabilizes coexistence, since every species will exert stronger intra- than interspecific competition. However, values of $l$ vary among species to create a trade-off between growth rates and competitive ability and low temperature tolerance: fast-growing species (high $g$) are more tolerant of interspecific competition (low $l$) but are more limited by temperature (high $Kmin$).
