...
\begin{table}[htp]
	...
	\begin{tabular}{llrl}
		...
		final\_yrs & 2000  & Number of yrs at steady-state, elevated temperature \\
		\hline
...
\begin{table}[htp]
	\caption{\label{tab:eco-evo-pars}Parameters and parameter values for the eco-evolutionary case study. Values are assigned at
		at the start of \texttt{genetic\_diversity\_master.R}. ``Name" refers to the variable declared in the computer code.  }
	\centering
	\smallskip 
	\begin{tabular}{llrl}
		\hline
		...
		\hline
		fec\_Tmu & -1,0,1  & Optimal temperature for genotypes AA, Aa, and aa \\
		fec\_Tsigma & 8  & Standard deviation in temperature niche for all genotypes \\
		fec\_max & 100 &  Maximum fecundity for all genotypes \\
		G & 1 & Germination rate for all genotypes \\
		seedSurv &  0.5  & Survival rate for ungerminated seeds of all genotypes \\
		alpha & 1  &  All competition coefficients (intra- and inter-) for all genotypes \\
		\hline
