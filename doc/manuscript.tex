...
an eco-evolutionary scenario. Our \comment{simulations illustrate} that: 
\begin{enumerate}
	\item For short-term forecasts, phenomenological time-series approaches \comment{may be} hard to beat, whereas longer-term forecasts \comment{may} require accounting for the influence of slow processes such as evolutionary and ecological selection as well as dispersal.
	\item Different kinds of data reflect the operation of different processes: longitudinal data capture autocorrelation and fast responses of current assemblages to interannual environmental variation, while data spanning spatial gradients capture the long-term outcome of interactions between fast and slow processes.  Whether predictive models should be trained using longitudinal or spatial data sets, or both, \comment{likely} depends on the time-scale of the desired forecast.
...
\end{equation}
In the eco-evolutionary example, the response variable is the log of the population growth rate. The regression, \comment{which includes a quadratic effect of temperature,} is 
\begin{equation}
