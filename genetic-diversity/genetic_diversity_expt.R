...
###
### 1. See how rate of shift in models weights responds to
...
###
### 2. Do replicates of the run featured in main text
###
reps <- 10
saved_weights_reps <- matrix(NA,nrow=length(weight),ncol=reps)
fec_Tsigma <- fec_Tsigma_baseline
...
for(iRun in 1:reps){
  ...
  saved_weights_reps[,iRun] <- weight
  
}

colnames(saved_weights_reps)<-paste0("rep",1:reps)
write.csv(saved_weights_reps,"saved_weights_reps.csv",row.names = F)
